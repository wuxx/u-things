...
xyzModem_get_hdr (void)
{
    ...
    while (!hdr_found)
    {
        ...
        if (res)
        {
            ...
            if (g_store_addr == 0x20009800) { uart4_printf("%s-%d %d\n", __func__, __LINE__, c); }
            switch (c)
            {
                case SOH:
                    xyz.total_SOH++;
        ...
        else
        {
            ...
            if (g_store_addr == 0x20009800) { uart4_printf("%s-%d\n", __func__, __LINE__); }
    ...
    {
        res = CYGACC_COMM_IF_GETC_TIMEOUT (*xyz.__chan, &c);
        if (g_store_addr == 0x20009800) { uart4_printf("%s-%d %d %x %x\n", __func__, __LINE__, i, c, res); }
        ZM_DEBUG (zm_save (c));
        if (res)
        {
            xyz.pkt[i] = c;
